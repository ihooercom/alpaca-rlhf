...
import torch
import wandb
from ppo_trainer import DeepSpeedPPOTrainer
from ppo_trainer import DeepSpeedPPOTrainerUnsupervised
...
from alpaca_rlhf.deepspeed_chat.training.utils.data.data_utils import (
...
def parse_args():
    ...
    parser.add_argument("--unsup_coef", type=float, default=27.8, help="""gamma in Equation 2 from InstructGPT paper""")
    parser.add_argument(
    ...
    parser.add_argument(
        "--generation_batch_numbers", type=int, default=1, help="Generate x batches to go to training mode."
    )
    parser.add_argument(
        "--ppo_epochs", type=int, default=1, help="For generated data, how many ppo training epochs to run."
    )
    ...
    parser.add_argument("--actor_weight_decay", type=float, default=0.0, help="Weight decay to use.")
    parser.add_argument("--critic_weight_decay", type=float, default=0.0, help="Weight decay to use.")
    ...
    parser.add_argument(
        "--gradient_accumulation_steps", type=int, default=1, help="Number of steps for the warmup in the lr scheduler."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--offload_reference_model", action="store_true", help="Enable ZeRO Offload techniques for reference model"
    )
    parser.add_argument(
        "--actor_zero_stage", type=int, default=0, help="ZeRO optimization stage for Actor model (and clones)."
    )
    parser.add_argument(
        "--critic_zero_stage", type=int, default=0, help="ZeRO optimization stage for Critic model (and reward)."
    )
    parser.add_argument(
        "--actor_gradient_checkpointing", action="store_true", help="Enable HF gradient checkpointing for Actor model."
    )
    parser.add_argument(
    ...
    parser.add_argument("--disable_actor_dropout", action="store_true", help="Disable the dropout of the actor model.")
    parser.add_argument(
        "--disable_critic_dropout", action="store_true", help="Disable the dropout of the critical model."
    )
    ## LoRA for efficient training setting
    parser.add_argument("--actor_lora_dim", type=int, default=0, help="If > 0, use LoRA for efficient training.")
    parser.add_argument("--actor_lora_module_name", type=str, default="decoder.layers.", help="The scope of LoRA.")
    parser.add_argument("--critic_lora_dim", type=int, default=0, help="If > 0, use LoRA for efficient training.")
    parser.add_argument("--critic_lora_module_name", type=str, default="decoder.layers.", help="The scope of LoRA.")
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    data_collator = DataCollatorRLHF(args.max_prompt_seq_len, args.inference_tp_size, tokenizer.pad_token_id)
...
def main():
    ...
    tokenizer = LlamaTokenizer.from_pretrained(args.tokenizer_name_or_path, fast_tokenizer=False)
