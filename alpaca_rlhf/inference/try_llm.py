...
parser.add_argument(
    ...
    choices=["decapoda-research/llama-7b-hf", "facebook/opt-350m", "gpt2"],
)
...
if model_name == "gpt2":
    ...
    model = AutoModelForCausalLM.from_pretrained(model_name, from_tf=bool(".ckpt" in model_name), config=model_config)
...
def predict(input, chatbot, max_new_tokens, num_beams, do_sample, temperature, top_p, history):
    ...
    generation_config = GenerationConfig(
        max_new_tokens=max_new_tokens, num_beams=num_beams, do_sample=do_sample, temperature=temperature, top_p=top_p
    )
    with torch.no_grad():
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=4):
            with gr.Column(scale=12):
                user_input = gr.Textbox(show_label=False, placeholder="Input...", lines=7).style(container=False)
            with gr.Column(min_width=32, scale=1):
                submitBtn = gr.Button("Submit", variant="primary")
        with gr.Column(scale=1):
            emptyBtn = gr.Button("Clear History")
            max_new_tokens = gr.Slider(0, 1024, value=128, step=1.0, label="Maximum Token Number", interactive=True)
            num_beams = gr.components.Slider(minimum=1, maximum=4, step=1, value=1, label="Beams")
        with gr.Column(scale=1):
    ...
    submitBtn.click(
        predict,
        [user_input, chatbot, max_new_tokens, num_beams, do_sample, temperature, top_p, history],
        [chatbot, history],
        show_progress=True,
    )
